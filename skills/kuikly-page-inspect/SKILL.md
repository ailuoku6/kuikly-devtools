...
description: Inspect a live Kuikly page through kuikly-devtools when debugging UI structure, component state, device logs, or network requests. Use the paginated CLI search rather than loading a full page snapshot.
---
...
Start with `sessions`, choose a live `pagerId`, then query only the relevant data type. Use `--offset` and `--limit` to page through results. Logs accept `--level` (`i`, `d`, `e`, or `p`) and `--tag`; network accepts `--status` and `--kind`.
